...
const findShoppingListAndMailIt = async (householdId, userId) => {
  try {
      ...
      members.forEach(async (member) => {
        ...
        if (option.sendMailShoppingList) {
          ...
          switch (option.dateMailShoppingList.value) {
            case 0:
              findShoppingListAndMailIt(household._id, member.userData);
              break;
            case 1:
              if (numberWeek % 2 === 0) {
                findShoppingListAndMailIt(household._id, member.userData);
...
const findProductAndMailIt = async (
  householdId,
  userId,
  ...
  try {
    ...
    let products = await Product.find({ householdId: householdId }).populate(
      "brand",
      "brandName"
    );
    ...
    if (products.length >= 1) {
      await products.forEach((product) => {
        product.expirationDate.forEach((date) => {
          if (date.expDate < warningDate) {
            let searchProduct = productExpirationDate.find(
              (productInArray) => productInArray.id === product._id
            );
            if (searchProduct) {
      ...
      if (productExpirationDate.length >= 1) {
        let tr = productExpirationDate
          .map((product) => {
            let numberProduct = 0;
            let listDate = product.expirationDate
              .map((expDateObject) => {
                numberProduct += expDateObject.number;
                return `<li>${Moment(expDateObject.expDate).format(
                  "DD-MM-YYYY"
                )} - nombre ${expDateObject.number}</li>`;
              })
              .join("");
            return `<tr>
                    <td>${product.name}</td>
      ...
      members.forEach(async (member) => {
        ...
        if (option.sendMailGlobal) {
          switch (option.dateMailGlobal.value) {
            case 0:
              findProductAndMailIt(
                household._id,
                member.userData,
                option.warningExpirationDate.value
              );
              break;
            case 1:
