...
} = require("../socket-io-management-utils");

dbManagement();

describe("Add user", () => {
  it("1) Create two users with same email", async () => {
    await request(app).post(`/api/${api}/users`).send(adminOneDataComplete);
  ...
  it("2) Error test, add user with householdName/Code missing", async () => {
  ...
  it("3) Error test, add user with bad householdCode", async () => {
    const response = await request(app)
    ...
    expect(JSON.parse(response.error.text).isBoom).toBe(true);
    expect(response.body.output.payload.message).toBe(
      "Pas de famille trouvée avec ce code famille!"
    );
  });
  it("4) Error test, add user with householdCode from an isWaiting household", async () => {
    let isWaitingHousehold = new Household({
      members: [{ userData: "606dad080ac1c22766b37a53", isFlagged: false }],
    ...
    const response = await request(app)
      .post(
        `/api/${api}/users?householdCode=${isWaitingHousehold.householdCode}`
      )
      .send(userTwoDataComplete);
    ...
    expect(JSON.parse(response.error.text).isBoom).toBe(true);
    expect(response.body.output.payload.message).toBe(
      "Vous ne pouvez pas envoyer une requête d'invitation à cette famille car elle n'a, en ce moment, pas d'administrateur.trice!"
    );
  });
  it("5) Add user with household name and without other member code", async () => {
    const response = await request(app)
      .post(`/api/${api}/users`)
      .send(adminOneDataComplete);
  ...
  it("6) Add user with household name and bad other member codes (usercode doesn't exist and member is in isWaiting familly)", async () => {
    let userOne = await createUser({ userData: userTwoDataComplete });

    let newNotification = await new Notification({
      message:
        "Vous avez été désigné.e comme nouvel.le administrateur.trice de cette famille par l'ancien.ne administrateur.trice, acceptez-vous cette requête ou passez l'administration à un.e autre membre de votre famille. Attention si vous êtes le/la dernier.ère membre éligible de cette famille, la famille sera supprimée et ne pourra pas être récupérée!",
    ...
    expect(JSON.parse(response.error.text).isBoom).toBe(true);
    expect(response.body.output.payload.message).toBe(
      "Il y a un ou plusieurs problèmes avec certains de vos codes utilisateurs!"
    );
  });
  it("7) Add user with household name and good other member codes", async () => {
    ...
    let userOne = await createUser({
      clientSocket: clientSocketUserOne,
      userData: userTwoDataComplete,
    });
    let userTwo = await createUser({
      clientSocket: clientSocketUserTwo,
      userData: userFourDataComplete,
    });
    ...
    expect(updateNotifReceivedUserOne.message).toBe(
      `L'administrateur.trice de la famille ${household.householdName} vous invite à rejoindre sa famille. Acceptez-vous l'invitation?`
    );
    expect(updateNotifReceivedUserOne.type).toBe(
      `invitation-household-to-user`
    );

    expect(updateNotifArrayUserOne[0].arrayData[0]._id.toString()).toBe(
      updateNotifReceivedUserOne._id.toString()
    );
    expect(updateNotifArrayUserOne[0].arrayData[0].message).toBe(
      updateNotifReceivedUserOne.message
    );
    expect(updateNotifArrayUserOne[0].arrayData[0].type).toBe(
      updateNotifReceivedUserOne.type
    );

    expect(updateNotifReceivedUserTwo.message).toBe(
      `L'administrateur.trice de la famille ${household.householdName} vous invite à rejoindre sa famille. Acceptez-vous l'invitation?`
    );
    expect(updateNotifReceivedUserTwo.type).toBe(
      `invitation-household-to-user`
    );

    expect(updateNotifArrayUserTwo[0].arrayData[0]._id.toString()).toBe(
      updateNotifReceivedUserTwo._id.toString()
    );
    expect(updateNotifArrayUserTwo[0].arrayData[0].message).toBe(
      updateNotifReceivedUserTwo.message
    );
    expect(updateNotifArrayUserTwo[0].arrayData[0].type).toBe(
      updateNotifReceivedUserTwo.type
    );
  ...
  it("8) Add user with household code", async () => {
    let clientSocketAdminOne = Client(`http://localhost:8003`);
    ...
    clientSocketAdminOne.emit("enterSocketRoom", {
      socketRoomName: responseAdminOne.body._id,
    });
    clientSocketAdminOne.emit("enterSocketRoom", {
      socketRoomName: `${responseAdminOne.body._id}/notificationReceived/0`,
    });
    ...
    const householdCreated = await Household.findById(
      responseAdminOne.body.householdId
    );
    ...
    expect(updateNotifReceivedAdminOne.message).toBe(
      `L'utilisateur.trice ${responseUserTwo.body.firstname} ${responseUserTwo.body.lastname} veut rejoindre votre famille. Acceptez-vous la demande?`
    );
    expect(updateNotifReceivedAdminOne.type).toBe(
      `invitation-user-to-household`
    );

    expect(updateNotifArrayAdminOne.arrayData[0]._id.toString()).toBe(
      updateNotifReceivedAdminOne._id.toString()
    );
    expect(updateNotifArrayAdminOne.arrayData[0].message).toBe(
      updateNotifReceivedAdminOne.message
    );
    expect(updateNotifArrayAdminOne.arrayData[0].type).toBe(
      updateNotifReceivedAdminOne.type
    );
